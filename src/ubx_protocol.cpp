...
bool UBX_Port::read_packet(ubx_packet_t *packet)
{
  ...
  if(_debug) {
    printf("ubx_read (%i):\t", data_len+6);
    for(int i=0; i<4; i++) printf("%02X ", buffer[i]);
    printf("|");
    for(int i=4; i<data_len+4; i++) printf("%02X ", buffer[i]);
    printf("|");
    for(int i=data_len+4; i<data_len+6; i++) printf("%02X ", buffer[i]);
    printf("\n");
  }
...
//-----------------------------------------------------------------------------
void UBX_Port::write_packet(ubx_packet_t *packet)
{
  ...
  if(_debug) {
    printf("ubx_write (%i):\t", data_len+6);
    for(int i=2; i<6; i++) printf("%02X ", buffer[i]);
    printf("|");
    for(int i=6; i<data_len+6; i++) printf("%02X ", buffer[i]);
    printf("|");
    for(int i=data_len+6; i<data_len+8; i++) printf("%02X ", buffer[i]);
    printf("\n");
  }
